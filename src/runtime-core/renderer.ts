...
function patchKeyedChildren(c1: any[], c2: any[], container) {
	...
	while (i <= e1 && i <= e2) {
		...
		if (!isSameVNodeType(prevChild, nextChild)) {
			console.log("俩个 child 不相等（从左往右对比）");
			break;
		}

		console.log("俩个child 相等， 接下来对比 俩个 children 节点");
	...
	while (i <= e1 && i <= e2) {
		...
		if (!isSameVNodeType(prevChild, nextChild)) {
			console.log("俩个 child 不相等（从右往左对比）");
			break;
		}
